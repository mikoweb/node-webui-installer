...
function vendorDirectory (settings) {
    ...
    if (settings.vendor_dir === null) {
        dir = cwd + '/' + settings.directory + '/' + paths.vendor;
    } else {
        dir = cwd + '/' + settings.vendor_dir;
    }
        ...
        if (onlyVendor) {
            ...
            tmp.dir({prefix: 'webui_'}, function (err, path) {
                ...
                bower.on('exit', function () {
                    ...
                    if (typeof settings.public_dir === 'string' && settings.public_dir.length 
                        && fs.existsSync(webuiFiles + '/public')
                    ) {
                        fsExtra.copySync(webuiFiles + '/public', cwd + '/' + settings.public_dir);
                    }
