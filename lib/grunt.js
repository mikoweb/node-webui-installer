...
module.exports = {
    ...
    execute: function (settings) {
        var webuiVendor = bower.vendorDirectory(settings);
        ...
        tmp.dir({prefix: 'webui_grunt_'}, function (err, path) {
            ...
            var vendorDir = bower.bowerDirectory(settings),
                bowerCwd = bower.bowerCwd(settings),
            ...
            if (fs.existsSync(bowerCwd + '/webui-grunt.json')) {
                ...
                gruntContent = gruntContent
                    .replace(/\{\{path_webui_vendor\}\}/g, JSON.stringify(webuiVendor + '/'))
                    .replace(/\{\{path_package_js\}\}/g, JSON.stringify(__dirname + '/../package.json'))
